...
class ShapeRenderer: NodeRenderer {
  ...
  fileprivate func drawPath(fill: Fill?, stroke: Stroke?, ctx: CGContext?, opacity: Double, fillRule: FillRule) {
    ...
    if let patternFill = fill as? Pattern, let stroke = stroke, let ctx = ctx {
      // Save the current path for stroke
      guard let path = ctx.path else { return }

      // Draw pattern fill first
      ctx.saveGState() // Save state before clipping for pattern
      setFill(patternFill, ctx: ctx, opacity: opacity)
      ctx.restoreGState() // Restore state to remove clipping
    ...
    if let fill = fill, let stroke = stroke, let ctx = ctx {
      guard let path = ctx.path else { return }
